...
export default function PremiumFeatures({ language }: PremiumFeaturesProps) {
  ...
  const achievements = [
    {
      icon: Award,
      title: { fr: 'Prix Excellence 2024', ar: 'جائزة التميز 2024' },
      subtitle: { fr: 'Meilleur Site Touristique', ar: 'أفضل موقع سياحي' }
    },
    {
      icon: Users,
      title: { fr: '50,000+', ar: '50,000+' },
      subtitle: { fr: 'Clients Satisfaits', ar: 'عميل راضي' }
    },
    {
      icon: Star,
      title: { fr: '4.9/5', ar: '4.9/5' },
      subtitle: { fr: 'Note Moyenne', ar: 'التقييم المتوسط' }
    },
    {
      icon: Shield,
      title: { fr: '100%', ar: '100%' },
      subtitle: { fr: 'Sécurisé', ar: 'آمن' }
    }
  ];
        ...
        <div className="text-center mb-16">
          <div className={`flex items-center justify-center gap-3 mb-6 ${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`}>
            ...
            <h2 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-morocco-gold to-yellow-300 bg-clip-text text-transparent">
              {language === 'fr' ? 'Fonctionnalités Premium' : 'ميزات متميزة'}
            </h2>
            <Sparkles className="h-8 w-8 text-morocco-gold animate-pulse" />
          </div>
        ...
        <div className={`bg-gradient-to-r from-morocco-red/20 to-morocco-gold/20 rounded-3xl p-8 backdrop-blur-sm ${isVisible ? 'animate-fade-in-up animation-delay-600' : 'opacity-0'}`}>
          <div className="text-center mb-8">
            ...
            <p className="text-gray-300">
              {language === 'fr' ? 'Des chiffres qui parlent d\'eux-mêmes' : 'أرقام تتحدث عن نفسها'}
            </p>
          </div>
        ...
        <div className={`text-center mt-16 ${isVisible ? 'animate-fade-in-up animation-delay-1000' : 'opacity-0'}`}>
          <div className="bg-gradient-to-r from-morocco-gold to-yellow-400 rounded-2xl p-8 text-black">
            <h3 className="text-3xl font-bold mb-4">
              {language === 'fr' ? 'Prêt à Vivre l\'Excellence ?' : 'مستعد لتجربة التميز؟'}
            </h3>
            ...
            <button className="bg-black text-white hover:bg-gray-800 px-8 py-4 rounded-xl font-bold transition-all duration-300 transform hover:scale-105 flex items-center gap-3 mx-auto group">
              {language === 'fr' ? 'Découvrir Maintenant' : 'اكتشف الآن'}
